...
2. **Data Dictionary (Markdown)**:
   ...
   - **MANDATORY surrogate-key naming convention (both the DDL and the downstream engineer depend on this being followed EXACTLY):** every dimension's surrogate key column MUST be named `<entity>_sk`, where `<entity>` is the dimension's singular table stem with the `dim_` prefix removed (e.g. `dim_customer` → `customer_sk`, `dim_accommodations` → `accommodation_sk`, `dim_components` → `component_sk`). Declare it as `BIGINT`. This `<entity>_sk` column is the ONLY new key column the dimension introduces; the natural/business key keeps its EXACT silver column name (do NOT rename it, do NOT add a `_bk` suffix). Every fact table references a dimension through that same `<entity>_sk` foreign key column. Use this identical `<entity>_sk` name in the DDL, the data dictionary, and (for the engineer) the `scd2_merge` surrogate_key_col argument and all fact joins — never mix `_sk`, `_key`, and `_id` for the same key.
...
2. **silver.py**:
   ...
   - **Return contract of `validate_table` — obey exactly**: it returns a 3-tuple `(clean_df, quarantined_df, report)`. `clean_df` and `quarantined_df` are Spark DataFrames (use `.count()` on THESE for row counts). `report` is a plain Python **dict**, NOT a DataFrame — read values by KEY: `report["row_count"]`, `report["clean_count"]`, `report["quarantined_count"]`, `report["promotion_blocked"]` (bool), `report["blocking_reason"]` (str), `report["rule_results"]` (list). NEVER call `.count()` (or any DataFrame method) on `report`, on a `contract`, or on the `contracts`/`parent_dfs` dicts — doing so raises `AttributeError: 'dict' object has no attribute 'count'` and kills the script. Unpack all three values (`clean_df, quarantined_df, report = validate_table(...)`); do not assign the whole tuple to one variable. For the summary counts below, prefer the report keys (`report["clean_count"]`, `report["quarantined_count"]`) or `.count()` on the DataFrames — never on the dict.
   - Runs validation in dependency order (parents before children) so child referential checks can query silver parent tables.
   - Handles boolean mappings (standardizing Y/N/Yes/No indicators to booleans) AFTER contract validation checks but BEFORE writing to Silver. Note: Do NOT use pandas `applymap` as it is not supported on PySpark DataFrames. Instead, use a loop over `df.dtypes` to find string columns and apply `when`/`otherwise` with PySpark's `col` and `lit`, calling `.isin(...)` as a method on the Column (e.g. `col("flag").isin("Y", "Yes")`) to map Yes/Y to True and No/N to False. IMPORTANT: `isin` is a Column METHOD, not a function — never write `from pyspark.sql.functions import isin` (it does not exist there and will raise ImportError).
   - **Crucial Requirement — write ordering**: `write_full_overwrite(clean_df, "silver", table_name)` must be the LAST thing done to `clean_df` for that table, strictly AFTER the boolean mapping step above reassigns `clean_df`. Writing Silver first and applying the boolean mapping afterward silently discards the conversion — the persisted Silver table would keep raw `'Y'`/`'N'` string values instead of real booleans, with no error raised anywhere to catch it. Write invalid rows to Quarantine (`write_full_overwrite(quarantine_df, "quarantine", table_name)`) at either point since quarantine rows are unaffected by the boolean mapping.
   ...
   - **Crucial Requirement**: If `halted_at` is not null (i.e. the loop broke early because a table's hard rule failure rate was exceeded), you MUST `raise RuntimeError(...)` with a message naming the table AND the exact blocking rule, AFTER writing `/tmp/silver_summary.json` — do not let the script finish normally. The report dict returned by `validate_table(...)` contains a `blocking_reason` key that names precisely which hard rule(s) failed and at what rate (e.g. `not_null[postcode] failed 12.50% (> allowed 0.00%)`); include `report['blocking_reason']` verbatim in the RuntimeError message so the failure is self-explanatory (e.g. `raise RuntimeError(f"Promotion blocked at {table_name}: {report['blocking_reason']}")`). Also store that string in the per-table silver_summary entry under a `"blocking_reason"` key. This is not optional: the orchestrator (which runs this script via `exec()` and only catches `Exception`, not `SystemExit`) only stops the pipeline and skips `gold.py` when this script raises. Do NOT call `sys.exit()` for this — it raises `SystemExit`, which the orchestrator's harness does not catch, so use `raise RuntimeError(...)` instead. If silver.py halts but returns normally, the pipeline will proceed to run gold.py against an incomplete Silver layer (tables after the halt point were never written), and gold.py will crash with a confusing `TABLE_OR_VIEW_NOT_FOUND` error instead of clearly reporting the real cause — a Silver contract validation failure.
3. **gold.py**:
   - Reads silver tables.
   ...
   - **Crucial Requirement — building fact tables**: For EACH fact table, call `read_table('silver', '<that specific table name>')` fresh and directly. Do NOT reuse a DataFrame variable left over from a loop that built dimensions for a DIFFERENT table — after a `for table_name in source_tables:` loop finishes, any variable assigned inside that loop (e.g. `silver_df`) holds only whatever table was processed LAST, not a combined view of everything. Do NOT assume any silver DataFrame has a column identifying which table it came from (e.g. `table_name`, `source_table`) — `read_table(schema, table)` returns ONLY that one table's own real columns, nothing else, unless you explicitly added such a column yourself earlier in the script. A concrete anti-pattern that has actually broken a production run: `fact_bookings = silver_df.filter(col('table_name') == 'bookings').join(...)` where `silver_df` was left over from a prior dimension-building loop iteration over a different table (e.g. suppliers) — this fails with `UNRESOLVED_COLUMN.WITH_SUGGESTION: table_name cannot be resolved` because no such column exists, AND even if it did, `silver_df` at that point holds the wrong table's data entirely. The correct code is simply `fact_bookings = read_table('silver', 'bookings').join(...)`.
   - Uses `scd2_merge` for SCD Type 2 dimensions to track changes. Note that `scd2_merge` has the signature: `scd2_merge(new_df: DataFrame, schema: str, table: str, business_key: str, tracked_cols: List[str], surrogate_key_col: str) -> str`. Make sure you pass all 6 arguments: `schema` is always `"gold"`, `table` is the gold dimension table name (e.g. `'dim_customer'`), `business_key` is the natural key column name (e.g. `'customer_id'`), `tracked_cols` is a list of column names to track changes for (e.g. `['first_name', 'last_name', 'email']`), and `surrogate_key_col` is the gold surrogate key name to generate. **`surrogate_key_col` MUST be exactly the `<entity>_sk` name declared for that dimension in the Gold DDL** (e.g. `'customer_sk'` for `dim_customer`, `'component_sk'` for `dim_components`) — NOT a `_key` or `_id` variant. Whatever name you pass here is the actual column that gets created on the dimension, so the fact-table join that resolves this dimension's foreign key MUST reference this SAME `<entity>_sk` name. A mismatch — e.g. building the dimension with `surrogate_key_col='component_key'` but then joining the fact on `dim.component_sk` — fails at runtime with `UNRESOLVED_COLUMN.WITH_SUGGESTION: component_sk cannot be resolved`. Pick the `_sk` name once from the DDL and use it verbatim in the scd2_merge call, the fact join condition, and the fact's selected foreign-key column.
   - **Crucial Requirement — dimension key column names are FIXED by the Gold DDL**: When joining a fact to a dimension to resolve its surrogate key, you MUST reference the dimension's key columns using the EXACT names declared in the Gold DDL above — both the surrogate key and the natural/business key. Do NOT invent a suffix convention. In particular, NEVER assume a dimension exposes a business key named `<entity>_bk` (e.g. `accommodation_bk`); that suffix is a hallucination that fails with `UNRESOLVED_COLUMN.WITH_SUGGESTION`. Read the actual column names off the DDL — the natural key is whatever the DDL calls it (e.g. `accommodation_ref`, `customer_id`), and the surrogate is always the `<entity>_sk` column the DDL declares (e.g. `accommodation_sk`, `customer_sk`). Before writing any `.join(dim, fact.<x> == dim.<y>)`, confirm both `<x>` and `<y>` appear verbatim in the DDL/silver schema; if a name is not in the schema, it does not exist — pick the real one from the DDL instead of guessing.
