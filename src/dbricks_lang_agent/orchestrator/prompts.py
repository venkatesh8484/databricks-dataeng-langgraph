...
2. **silver.py**:
   ...
   - Handles boolean mappings (standardizing Y/N/Yes/No indicators to booleans) AFTER contract validation checks but BEFORE writing to Silver. Note: Do NOT use pandas `applymap` as it is not supported on PySpark DataFrames. Instead, use a loop over `df.dtypes` to find string columns and apply `when`/`otherwise` with PySpark's `col`, `lit`, and `isin` to map Yes/Y to True and No/N to False.
